...
<input type="hidden" name ="memid" value="${dto.memId }"/>
    <table border = 1>
        <tr><td>아이디</td><td>${dto.memId }</td></tr>
        <tr><td>이름</td><td>${dto.memName }</td></tr>
        <tr><td>우편번호</td>
        ...
        <tr><td>수신여부</td>
            <td><input type="radio" name="memEmailCk" value="Y" <c:if test="${dto.memEmailCk  == 'Y'}">checked</c:if>> 넵
                <input type="radio" name="memEmailCk" value="N" <c:if test="${dto.memEmailCk  == 'N'}">checked</c:if>> 아뇨
            </td></tr>
        <tr><td colspan="2" align="center">
            <input type="submit" value="가입하기" />
            <input type="button" value="수정안함" onclick="javascript:histroy.back();"/>
            <input type="button" value="회원탈퇴" onclick="javascript:location.href='memDel.mem?memId=${dto.memId}'"/>
